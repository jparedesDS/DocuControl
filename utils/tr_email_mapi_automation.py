...
from utils.tr_data_process import *
...
while message:
    ...
    try:
        ...
        df = df.loc[:, ['Vendor Number', 'TR Number', 'Title', 'Vendor Rev', 'TR Rev',
                            'Return Status']]  # Reorganizamos las columnas para realizar la importación correcta a BBDD.
        df['Tipo de documento'] = df[
                'Vendor Number']  # Creamos una nueva columna en la cual identificamos el Tipo de documento a traves del ['Vendor Number'].
        df['Tipo de documento'] = df['Tipo de documento'].str.extract(r'(\w[A-Za-z#&]+)',
                                                                          expand=False)  # Obtenemos el 'TIPO DE DOCUMENTO'.
        df['Supp.'] = df[
                'Vendor Number']  # Creamos una nueva columna en la cual identificaremos el suplemento a traves del ['Vendor Number'].
