...
def reconocer_tipo_proyecto(df):
    ...
    """
    mapping = {'EP-PP29': 'TEMPERATURA'}

    # Asignamos el tipo de proyecto según el número de pedido
    df['Material'] = df['Paquete'].map(mapping).fillna(df['PO'])  # Si no se encuentra en el mapeo, deja el código original
