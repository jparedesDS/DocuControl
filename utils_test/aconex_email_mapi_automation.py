""" Versión v1.0.0 - Automatización, envío, tratamiento de datos y entrada a BBDD para transmittals de ACONEX """
# Imports
import time
import shutil
...
from utils.prodoc_data_process import *
...
nombre_carpeta = os.path.join(f'Z:\\JOSE\\02 DEVOLUCIÓN DOCUMENTACIÓN\\ACONEX\\' +dia)
...
combine_path = f'Z:\\JOSE\\02 DEVOLUCIÓN DOCUMENTACIÓN\\ACONEX\\all_tr_combine.xlsx'
...
dst = f'Z:\\JOSE\\02 DEVOLUCIÓN DOCUMENTACIÓN\\ACONEX\\' +str(dia)    # Generamos una url de una nueva carpeta en la que iran los .xlsx
...
while message:
    if message.SenderEmailAddress != 'noreply@aconex.com':
    ...
    try:
        ...
        df1 = df_list[0]
        print(df1)
        df1 = df1.loc[:, ['No. de documento', 'Revisión', 'Título', 'Status ', 'Date']]  # Reorganizamos las columnas para realizar la importación correcta a BBDD.
        print(df1)
