...
column_order = [
    ...
    'Título', 'Tipo Doc.', 'Crítico', 'Estado', 'Notas', 'Nº Revisión',
...
"""# === AÑADIR GRÁFICO DE TARTA EN "STATUS GLOBAL" ===
wb = load_workbook(output_path)
if "STATUS GLOBAL" in wb.sheetnames:
    ws = wb["STATUS GLOBAL"]

    # Buscar columnas de estados (excepto 'Nº Pedido', 'Total' y '% Completado')
    headers = [cell.value for cell in ws[1]]
    estado_cols = [h for h in headers if h not in ['Nº Pedido', 'Total', '% Completado']]

    # Calcular totales por estado
    totals = []
    for estado in estado_cols:
        col_idx = headers.index(estado) + 1
        total_estado = sum(ws.cell(row=i, column=col_idx).value or 0 for i in range(2, ws.max_row + 1))
        totals.append(total_estado)

    # Escribir resumen al final de la hoja (para vincular al gráfico)
    resumen_row = ws.max_row + 3
    ws.cell(row=resumen_row, column=1, value="Resumen por Estado")
    for i, estado in enumerate(estado_cols, start=1):
        ws.cell(row=resumen_row + i, column=1, value=estado)
        ws.cell(row=resumen_row + i, column=2, value=totals[i - 1])

    # Crear gráfico de tarta
    from openpyxl.chart import PieChart, Reference

    pie = PieChart()
    pie.title = "Distribución Global de Estados"
    labels = Reference(ws, min_col=1, min_row=resumen_row + 1, max_row=resumen_row + len(estado_cols))
    data = Reference(ws, min_col=2, min_row=resumen_row, max_row=resumen_row + len(estado_cols))
    pie.add_data(data, titles_from_data=True)
    pie.set_categories(labels)
    pie.style = 10  # estilo visual bonito
    pie.height = 7
    pie.width = 9

    # Insertar gráfico al lado derecho de la tabla principal
    ws.add_chart(pie, "J3")

    wb.save(output_path)
    print("✅ Gráfico de tarta añadido correctamente en 'STATUS GLOBAL'.")"""
...
def apply_excel_styles(archivo_excel):
    ...
    # === Colorear columna Responsable solo texto y negrita ===
    ...
    for sheet in workbook.worksheets:
        col_resp_idx = None
        for idx, cell in enumerate(sheet[1], start=1):
            if cell.value == "Responsable":
                col_resp_idx = idx
                break
        if col_resp_idx:
            for row in sheet.iter_rows(min_row=2):
                val = row[col_resp_idx - 1].value
                if val in responsables_colores:
                    color_hex = responsables_colores[val]
                    row[col_resp_idx - 1].font = Font(color=color_hex, bold=True)
