...
def show_excel_viewer(excel_path):
    ...
    try:
        ...
        viewer.title(f"Visualización de: {os.path.basename(excel_path)}")
        viewer.geometry("900x500")
        viewer.configure(bg="#f0f0f0")

        tree = ttk.Treeview(viewer, show="headings")
        ...
        messagebox.showerror("Error", f"No se pudo mostrar el Excel:\n{e}")
...
def run_script_and_copy_excel(script_path, generated_file, popup=None):
    try:
        ...
        if destination_folder:
            ...
            # NUEVAS LÍNEAS PARA MOSTRAR EL EXCEL EN UNA NUEVA VENTANA
            df = pd.read_excel(generated_file_path)
            show_excel_viewer(df, f"Vista previa - {os.path.basename(generated_file_path)}")
...
def open_devoluciones_popup():
    ...
    dc_img = load_logo("C:\\Users\\alejandro.berzal\\Desktop\\DATA SCIENCE\\DocuControl\\tools\\img\\docucontrol.png")
