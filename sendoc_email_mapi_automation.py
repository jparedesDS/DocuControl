""" Versión v1.0.0 - Automatización, envío, tratamiento de datos y entrada a BBDD para transmittals de SENDOC """
# Imports
import time
import shutil
...
from utils.test.ERPconn import *
from utils.email_identification import *
from utils.po_identification import *
from utils_test.sendoc_data_process import *
from utils.apply_style_email import *
...
nombre_carpeta = os.path.join(f'Z:\\JOSE\\02 DEVOLUCIÓN DOCUMENTACIÓN\\SENDOC\\' +dia)
...
combine_path = f'Z:\\JOSE\\02 DEVOLUCIÓN DOCUMENTACIÓN\\SENDOC\\all_tr_combine.xlsx'
...
dst = f'Z:\\JOSE\\02 DEVOLUCIÓN DOCUMENTACIÓN\\SENDOC\\' +str(dia)    # Generamos una url de una nueva carpeta en la que iran los .xlsx
...
while message:
    if message.SenderEmailAddress != 'adminsendoc@corporate.sener':
    ...
    try:
        ...
        df_final = df_final.reindex(['Nº Pedido', 'Supp.', 'Responsable', 'Cliente', 'Material', 'PO', 'Doc. EIPSA', 'Doc. Cliente', 'Título', 'Rev.', 'Estado', 'Tipo de documento', 'Crítico', 'Nº Transmittal', 'Fecha'], axis=1)
